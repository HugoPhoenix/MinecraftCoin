...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("minecraftcoin"))
        return false;
...
{
    // Convert minecraftcoin:// to minecraftcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("minecraftcoin://"))
    {
        uri.replace(0, 12, "minecraftcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MinecraftCoin.lnk";
}
...
{
    return GetAutostartDir() / "minecraftcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MinecraftCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  minecraftcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("MinecraftCoin-Qt"));
